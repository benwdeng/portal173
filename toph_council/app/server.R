...
server <- function(input, output, session) {
  ...
  # input$listOfDocuments comes from front end js in "www/sof-auth.js"
  session$userData$relevantFiles <- reactiveVal(NULL)
  
  observeEvent(input$listOfDocuments, {
    # set the signed in user
    documentNames <- list(filename = input$listOfDocuments)
    session$userData$relevantFiles(documentNames)
  ...
  dataStorage <- reactiveValues(
    ...
    fact_document = reactiveFileReader(intervalMillis = 1000, session = NULL, readFunc = readRDS, filePath = path_file_fact_document)
  )
  ...
  mod_portal173_server(id = "portal173_tab", values = dataStorage, users = users)
  mod_permitholder_server(id = "permitholder_tab", values = dataStorage, users = users)
