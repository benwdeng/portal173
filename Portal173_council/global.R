...
# Modules
source("modules/council_mod_overview.R")
source("modules/council_mod_overview_approve_matter.R")
source("modules/council_mod_overview_reject_matter.R")
source("modules/council_mod_portal173.R")
...
status_choices <- c("1 Engagement", "2 Drafting stage", "3 Signing stage", 
                    ...
                    "6 council approved")
type_of_agreement <- c("Lodge", "Draft", "Remove", "Amend")
type_of_document <- c("Certificate/s of Title", "Planning Permits", 
                      "Company search", "Plan of Subdivision", "Power of Attorney",
                      "Other documents")
vic_council_list <- as.list(unique(as.data.table(read.csv("inputs/Postcode_to_LGA_ratio_2011.csv"))[between(as.integer(POSTCODE), 3000, 3999),"LGA_NAME_2011"]))
names(vic_council_list) <- "Council"
postcode_to_lga_init <- as.data.table(read.csv("inputs/Postcode_to_LGA_ratio_2011.csv"))
postcode_to_lga_mapping <- postcode_to_lga_init[postcode_to_lga_init[,.(RATIO == max(RATIO)), by = POSTCODE]$V1][,.(POSTCODE,LGA_NAME_2011)]
...
META <- list(
  ...
  logo_lg     = "<strong>The Online Planning Hub (Council)</strong>",
