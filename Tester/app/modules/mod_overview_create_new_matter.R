...
create_new_matter_server <- function(session, input, output, create_matter, values){
  ...
  observeEvent(
    ...
    ignoreInit = TRUE, 
    {
      ...
      # Add firestore_id
      firestore_id_entry <- fromJSON(content(firestore_response,"text"))$name
      firestore_id_entry <- substring(
        firestore_id_entry, 
        gregexpr('documents', firestore_id_entry)[[1]][1]+23, 
        nchar(firestore_id_entry)
      )
